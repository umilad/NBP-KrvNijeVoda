...
export default function Licnost() {
    ...
    useEffect(() => {
        ...
        load();
    }, [id, token, location.pathname, location.state]);
    ...
    const handleUpdate = () => {
        if (!id) return;
        navigate(`/licnost/edit/${id}`, {
            state: { isVladar: location.state?.isVladar }
        });
    };
